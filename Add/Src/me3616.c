...
#include "usart.h"
#include "tim.h"
#define ME3616_WAKE_UP_HIGH	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_SET)
#define ME3616_WAKE_UP_LOW	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_RESET)
...
{
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_SET);	/* 拉高power引脚开机 */
	HAL_Delay(1000);
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_RESET);	/* 释放power引脚 */
}
...
{
    HAL_GPIO_WritePin(GPIOC, GPIO_PIN_4,GPIO_PIN_SET);	/* 拉高RESET引脚开始复位 */
	HAL_Delay(1000);
    HAL_GPIO_WritePin(GPIOC, GPIO_PIN_4,GPIO_PIN_RESET);	/* 释放RESET引脚 */
}
...
{
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_SET);	/* 拉高WAKE_UP引脚，唤醒模组 */
	HAL_Delay(100);
...
void me3616_at_response(u8 mode)
{
    u8 tmpRecv[64] = {0};
    u16 revLen = 0 ;
    if(__HAL_UART_GET_FLAG(&USART2_Handler, UART_IT_TC)) /* 接收到一次数据了 */
    {
        memset(tmpRecv,0,sizeof(tmpRecv));
        revLen = UART2Recv(tmpRecv,sizeof(tmpRecv));
        printf("%s\r\n",tmpRecv);
    }
}
/**************************************************************
...
{
		if(__HAL_UART_GET_FLAG(&USART2_Handler,UART_IT_TC))
		{
			if(strstr((const char*)USART2_RX_BUF, "+MIPLEVENT") && (0 == g_me3616_miplnotify_flag))
			{
				if(strstr((const char*)USART2_RX_BUF, "+MIPLEVENT: 0, 25") || strstr((const char*)USART2_RX_BUF, "+MIPLEVENT: 0, 8")  \
				|| strstr((const char*)USART2_RX_BUF, "+MIPLEVENT: 0, 7") || strstr((const char*)USART2_RX_BUF, "+MIPLEVENT: 0, 5")   \
				|| strstr((const char*)USART2_RX_BUF, "+MIPLEVENT: 0, 9") || strstr((const char*)USART2_RX_BUF, "+MIPLEVENT: 0, 10")  \
				|| strstr((const char*)USART2_RX_BUF, "+MIPLEVENT: 0, 12") || strstr((const char*)USART2_RX_BUF, "+MIPLEVENT: 0, 13") \
				|| strstr((const char*)USART2_RX_BUF, "+MIPLEVENT: 0, 20") || strstr((const char*)USART2_RX_BUF, "+MIPLEVENT: 0, 3"))
				{
...
//					TIM_Cmd(TIM3, DISABLE);  /* 关闭TIMx */
				...
				else if(strstr((const char*)USART2_RX_BUF, "+MIPLEVENT: 0, 14"))
				{
					me3616_onenet_miplupdate(g_opend_lifetime, 1);
				}
				me3616_at_response(1);
			...
			else if(strstr((const char*)USART2_RX_BUF, "+MIPLEVENT: 0, 4") && (1 == g_me3616_miplnotify_flag))
			{
			...
			#endif
			else if(strstr((const char*)USART2_RX_BUF, "+MIPLOBSERVE"))
			{
			...
			else if(strstr((const char*)USART2_RX_BUF, "+MIPLDISCOVER"))
			{
			...
			else if(strstr((const char*)USART2_RX_BUF, "+CIS ERROR"))
			{
				me3616_at_response(1);
				g_hardware_times = 0;
//				TIM_Cmd(TIM3, DISABLE);  	/* 关闭TIMx */
			...
			else if(strstr((const char*)USART2_RX_BUF, "auto-reboot"))/* 模组发生异常重启现象 */
			{
				me3616_at_response(1);
				g_hardware_times = 0;
//				TIM_Cmd(TIM3, DISABLE);  /* 关闭TIMx */
			...
			else if(strstr((const char*)USART2_RX_BUF, "+IP"))	/* 查到IP，说明模组开机或者重启后注册上网络了 */
			{
				me3616_at_response(1);
...
{
	if(__HAL_TIM_GET_FLAG(&htim21, TIM_FLAG_UPDATE) != RESET)  /* 检查TIM3更新中断发生与否 */
	{
		__HAL_TIM_CLEAR_FLAG(&htim21, TIM_FLAG_UPDATE);  /* 清除TIMx更新中断标志 */
